...
#from flag import FLAG # type: ignore
from tqdm import tqdm 
from itertools import product

def generate_hexadecimals(length=8):
    digits = '0123456789abcdef'
    for hex_digit in product(digits, repeat=length):
        yield ''.join(hex_digit)
...
def slide():
	for _ in tqdm(range(2**32 )):
		p_1 = p_0[4:] + os.urandom(4)
		c_1 = cipher.encrypt(p_1)
		#k_1 = cipher.f(bytes_to_long(c_1[8:]) ^ bytes_to_long(c_0[:8])) ^ bytes_to_long(c_0[8:])
		if (c_0[4:] == c_1[:4]):
			k_0 = cipher.f(bytes_to_long(p_1[4:]) ^ bytes_to_long(p_0[:4])) ^ bytes_to_long(p_0[:4])
			return long_to_bytes(k_0)
